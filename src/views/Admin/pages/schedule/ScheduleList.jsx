...
const ScheduleList = () => {
  ...
  return (
    ...
    <div class="container-full">
      ...
      <div className="schedule-filter-right">
        ...
        <div className="schedule-filter-item">
          <label htmlFor="doctorId" className="schedule-filter-label">Doctor</label>
          <select
            id="doctorId"
            value={doctorId}
        ...
        {/* Select SpecialtyId */}
        <div className="schedule-filter-item">
          <label htmlFor="specialtyId" className="schedule-filter-label">Specialty</label>
          <select
            id="specialtyId"
            value={specialtyId}
        ...
        {/* Date Schedule */}
        <div className="schedule-filter-item">
          <label htmlFor="dateSchedule" className="schedule-filter-label">Date Schedule</label>
          <input
            type="date"
            id="dateSchedule"
            value={dateSchedule}
                                ...
                                <table class="table border-no" id="example1">
                                    ...
                                    <tbody>
                                        {schedules?.map((schedule)=>(
                                              ...
                                              <td>{convertToDateString(schedule?.date_schedule)}</td>
                                              <td>{convertToTimeString(schedule?.start_time)}</td>
                                              <td>{schedule?.booking_limit}</td>
                                              <td>
                                                  <div class="btn-group acion-admin-list" >
                                                      <a style={displayStyle}  onClick={() => navigate(`/admin/schedules/${schedule?.id}`)}>  <FontAwesomeIcon icon={faEye}  className='icon-actionAD'/></a>
                                                      <a style={displayStyle} onClick={() => navigate(`/admin/schedules/update/${schedule?.id}`)}> <FontAwesomeIcon icon={faPen} className='icon-actionAD'/></a>
    ...
    {openDeleteModal && <DeleteLayout  open={openDeleteModal} handleClose={handleDeleteClose} idObject={idObject} deleteFunction={deleteSchedule} onDelete={handleLoading}/>}
