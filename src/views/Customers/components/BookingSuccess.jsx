...
const BookingSuccess = () => {
  ...
  useEffect(() => {
    if (location.pathname.includes('/booking/result/') && idBooking && vnp_TransactionNo && vnp_ResponseCode === "00") {
      payOrderSuccess(idBooking, vnp_TransactionNo, vnp_ResponseCode);

      async function payOrderSuccess(
        bookingId,
        vnp_TransactionNo,
        vnp_ResponseCode
        ...
        try {
          const response = await All_API.payBookingSuccess(
            bookingId,
            vnp_TransactionNo,
            vnp_ResponseCode
          );
          if (response?.data.status === "success") {
            ...
            navigate(`/booking/success?bookingId=${idBooking}`)
          } else {
  ...
  }, [idBooking, vnp_TransactionNo, vnp_ResponseCode]);
    ...
    <div className="appointment-container mt-5">
      ...
      {statusPay === "Pay_failed" ? (
        <>
          ...
          <div className="box-appoint-note">
            <p className="appointment-note">
              Note: Your payment was not successful. Please try again or contact
              support.
            </p>
          </div>
          <div className="appointment-btn-sc">
            <button className="btn btn-main btn-round-full btn-book-conf ic-appoint-sc" onClick={()=> navigate('/')}>
            Return to Home!
        ...
        <>
          ...
          <div className="box-appoint-note">
            <p className="appointment-note">
              Note: Your appointment has been transferred to the medical
              facility. Please do not book through other channels to avoid
              duplicate schedules.
            </p>
          </div>
          <div className="appointment-btn-sc">
